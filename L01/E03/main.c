...
void InsertionSort_per_codice(struct corsa V[], int N);

void InsertionSort_per_Spartenza(struct corsa V[], int N);

void InsertionSort_per_Sarrivo(struct corsa V[], int N);

void InsertionSort_per_data(struct corsa V[], int N);

void ricerca_partenza_lineare(struct corsa corse[], int N, char stazione[]);
...
int selezionaComando(int N, struct corsa corse[]) {
    int comando = 0, scelta = 0, ordinato = 0;
    char stazione[30];
    ...
    while (comando != 6) {
        ...
        switch (comando) {
            case 1:
                InsertionSort_per_data(corse, N);
                stampa(corse, N);
                break;

            case 2:
                InsertionSort_per_codice(corse, N);
                stampa(corse, N);
                break;

            case 3:
                InsertionSort_per_Spartenza(corse, N);
                stampa(corse, N);
                ordinato = 1;
                break;

            case 4:
                InsertionSort_per_Sarrivo(corse, N);
                stampa(corse, N);
                break;
            ...
            case 5:
                ...
                scanf("%s", &stazione);
                if (ordinato)
                    ricerca_partenza_dicotomica(corse, N, stazione);
                else
                    ricerca_partenza_lineare(corse, N, stazione);
                break;
        }
    }
}
...
void InsertionSort_per_data(struct corsa V[], int N) {
    ...
    for (i = l + 1; i <= r; i++) {
        x = V[i];
        j = i - 1;
        while (j >= l && strcmp(x.ora_partenza, V[j].ora_partenza) < 0) {
            V[j + 1] = V[j];
            j--;
        }
        V[j + 1] = x;
    ...
    for (i = l + 1; i <= r; i++) {
        x = V[i];
        j = i - 1;
        while (j >= l && strcmp(x.data, V[j].data) < 0) {
            V[j + 1] = V[j];
            j--;
        }
        V[j + 1] = x;
...
void InsertionSort_per_Spartenza(struct corsa V[], int N) {
    ...
    for (i = l + 1; i <= r; i++) {
        x = V[i];
        j = i - 1;
        while (j >= l && strcmp(x.partenza, V[j].partenza) < 0) {
            V[j + 1] = V[j];
            j--;
        }
        V[j + 1] = x;
...
//ordinamento per stazioni di arrivo
void InsertionSort_per_Sarrivo(struct corsa V[], int N) {
    int i, j, l = 0, r = N - 1;
    struct corsa x;
    for (i = l + 1; i <= r; i++) {
        x = V[i];
        j = i - 1;
        while (j >= l && strcmp(x.destinazione, V[j].destinazione) < 0) {
            V[j + 1] = V[j];
            j--;
        }
        V[j + 1] = x;
...
//ricerca lineare parziale
void ricerca_partenza_lineare(struct corsa corse[], int N, char stazione[]) {
    int trovato = 1;

    for (int i = 0; i < N; i++) {
        trovato = 1;
        for (int j = 0; j < strlen(stazione) && trovato; j++) {
            if (corse[i].partenza[j] != stazione[j])
                trovato = 0;
        }
        if (trovato)
            printf("%s %s %s %s %s %s %d\n", corse[i].codice, corse[i].partenza, corse[i].destinazione,
